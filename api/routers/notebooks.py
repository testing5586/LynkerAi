...
async def get_notebooks(
    ...
    try:
        ...
        # Known DB-layer error: return 503 so the UI can display a friendly message
        logger.error(f"Database operation error while fetching notebooks: {str(e)}")
        raise HTTPException(status_code=503, detail=f"Database unavailable: {str(e)}")
    except ConnectionRefusedError as e:
        # Low-level connection refused from DB client (e.g. SurrealDB not running)
        logger.error(f"Connection refused while fetching notebooks: {str(e)}")
        raise HTTPException(status_code=503, detail=f"Database unavailable: {str(e)}")
